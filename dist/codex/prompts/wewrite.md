...
- `wewrite` 命令不存在或 `summary.failures > 0` → 引导 `bash {skill_dir}/install.sh`（安装/更新 wewrite CLI 并链接 skills）；否则静默继续。
...
2. **目标平台**：用户点名了就用；没点名 → 列出 `{skill_dir}/src/wewrite/toolkit/platforms/` 下的可用
...
1. `读取: {skill_dir}/src/wewrite/toolkit/platforms/<id>.yaml`，按其 `rewrite_brief`、字数区间、
   标签数、输出格式改写。
